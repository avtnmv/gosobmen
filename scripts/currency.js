...
function initReceiveOptions() {
    ...
    const firstOption = document.querySelector("#receive-select-options .custom-select__option");
    if (firstOption) {
        const iconSrc = firstOption.querySelector('img').src;
        const text = firstOption.querySelector('.custom-select__text').textContent;
        selected.innerHTML = `
            <div class="currency-preview">
                <img src="${iconSrc}" class="custom-select__icon" alt="${firstOption.dataset.value}-icon">
                <span class="custom-select__text">${text}</span>
        ...
        `;
        selected.dataset.value = firstOption.dataset.value;
    }
}
    ...
    if (receiveOptions[selectedCurrency]?.[0]) {
        ...
        selected.innerHTML = `
            <div class="currency-preview">
                ...
                <span class="custom-select__text">${firstOption.text}</span>
                <svg class="header__custom-select-arrow" viewBox="0 -4.5 20 20" xmlns="http://www.w3.org/2000/svg">
                    <path d="M20 1.39 18.594 0 9.987 8.261l-.918-.881.005.005L1.427.045 0 1.414 9.987 11z" fill-rule="evenodd"/>
                </svg>
            </div>
        ...
        if (selectedOption) {
            switch(type) {
                ...
                case 'receive':
                    ...
                    const text = selectedOption.querySelector('.custom-select__text').textContent;
